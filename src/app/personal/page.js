  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        console.log(
          "Personal page: Fetching dashboards with token:",
        ...
        if (dashboardId) {
          console.log("Personal page: Using dashboard ID from URL:", dashboardId);
          selectedDashboardId = dashboardId;
        } else {
          // Otherwise, auto-select personal dashboard from list
          const dashboardsResponse = await api.fetchDashboards(10, 0);
          console.log(
            "Personal page: Dashboards response:",
          ...
          const personalDashboards = dashboardsResponse.data.dashboards.filter(
            (dashboard) =>
          ...
          // Get the first owned personal dashboard
          const personalDashboard = personalDashboards[0];

          if (!personalDashboard) {
            console.log(
              "Personal page: No personal dashboard found. Available dashboards:",
              dashboardsResponse.data.map((d) => ({
                id: d.id,
                name: d.name,
                category: d.category,
              }))
          ...
          console.log(
            "Personal page: Found personal dashboard:",
            JSON.stringify(personalDashboard, null, 2)
          );
          selectedDashboardId = personalDashboard.id;
        }
        ...
        if (dashboardData.data?.widgets) {
          dashboardData.data.widgets.forEach((widget, index) => {
            console.log(`Personal page: Widget ${index}:`, {
              id: widget.id,
              name: widget.name,
        ...
        if (dashboardData.data?.dashboard_attributes) {
          dashboardData.data.dashboard_attributes.forEach((attr, index) => {
            console.log(`Personal page: Attribute ${index}:`, {
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-1">
          <h1 className="text-2xl font-semibold text-[#4D4D4D]  mb-1">
            {data.name || "Personal Dashboard"}
          </h1>
          {/* Heatmap button commented out - Heatmap widgets display automatically when added */}
          {/* <button
        ...
        <p className="text-sm font-medium text-[#4D4D4D]">
          {data.description ||
            "Manage your personal life, goals, and lifestyle."}
        </p>
      </div>

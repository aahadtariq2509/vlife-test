...
const HeatmapChartWidget = ({ chartData: initialChartData, title = 'Calorie Intake Logging' }) => {
  ...
  console.log('HeatmapChartWidget - chartData:', chartData);
  console.log('HeatmapChartWidget - series:', chartData.series);
  ...
  const currentSeries = chartData.series.map(s => ({
    ...
    unit: s.unit || 'kcal' // Get unit from series data, default to kcal
  }));

  // Calculate summary totals and target totals for each month in current quarter
  const summaryTotals = new Array(3).fill(0);
  const summaryTargets = new Array(3).fill(0);
  currentSeries.forEach((s) => {
    s.data.forEach((v, index) => {
      summaryTotals[index] += v;
    });
    // Sum up all targets for summary
    summaryTargets.fill(s.budget + (summaryTargets[0] || 0));
  });

  // Calculate total target for summary (sum of all meal targets)
  const totalTarget = currentSeries.reduce((sum, s) => sum + s.budget, 0);

  // Helper function to get cell color based on percentage of target
  // For expenses: LOWER spending is BETTER (green), HIGHER spending is WORSE (red)
  const getCellColor = (value, budget) => {
    if (budget === 0) return '#F3F4F6'; // Gray for no budget

    const percentage = (value / budget) * 100;

    // REVERSED logic for expenses: low percentage = good (green), high percentage = bad (red)
    if (percentage <= 50) return '#16A34A'; // Green: 0-50% (well under budget = good!)
    if (percentage <= 75) return '#FDE047'; // Yellow: 50-75% (moderate spending)
    if (percentage <= 90) return '#F59E0B'; // Orange: 75-90% (approaching budget limit)
    return '#EF4444'; // Red: 90-100%+ (at or over budget = bad!)
  };
  ...
  return (
    <Card className="rounded-3xl p-6 bg-white shadow-sm border border-gray-100">
          ...
          <tbody>
            ...
            <tr className="bg-gray-50">
              ...
              {summaryTotals.map((total, index) => {
                // Get unit from first series (all series should have same unit for calorie intake)
                const unit = currentSeries[0]?.unit || 'kcal';
                // Calculate color based on total target percentage
      ...
      {/* Progress Legend - REVERSED for expenses (low spending = green/good) */}
